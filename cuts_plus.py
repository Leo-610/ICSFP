...
class MultiCAD(object):
    ...
    def train(self, data, observ_mask, original_data, true_cm=None, predefined_G=None):
        """
        修改后的train函数，支持预定义分组

        Args:
            predefined_G: 预定义的分组矩阵，shape为(n_nodes, n_groups)
        """
        ...
        # 如果提供了预定义分组，使用它
        if predefined_G is not None and self.args.group_policy == 'predefined':
            print("Using predefined sector grouping...")
            self.G = torch.from_numpy(predefined_G).float().to(self.device)
            self.n_groups = predefined_G.shape[1]
            ...
            for g in range(self.n_groups):
                group_size = int(self.G[:, g].sum().item())
                print(f"Group {g}: {group_size} nodes")

        for epoch_i in range(self.args.total_epoch):

            # 原有的动态分组逻辑
            if self.args.group_policy is not None and self.args.group_policy != 'predefined':
            ...
            elif epoch_i == 0 and self.n_groups == self.args.n_nodes and predefined_G is None:
...
# 修改main函数以支持predefined_G参数
def main(data, mask, true_cm, opt, log, device="cuda", predefined_G=None):
    """
    修改后的main函数

    Args:
        predefined_G: 预定义的分组矩阵，如果提供则使用预定义分组
    """
    if opt.n_nodes == "auto":
    ...
    Graph = multicad.train(data, mask, data, true_cm, predefined_G=predefined_G)
    return Graph

...
class DataPipe:
        ...
        try:
            with io.open(stock_movement_path, 'r', encoding='utf8') as movement_f:
                for line in movement_f:  # descend
                    ...
                    if len(data) < 6:  # Ensure data integrity
                        continue
            ...
            logger.warning(f"Stock movement file not found: {stock_movement_path}")
            return None
        except Exception as e:
            logger.error(f"Error reading stock movement file {stock_movement_path}: {e}")
            return None
    ...
    def _get_unaligned_corpora(self, ss, main_target_date, vocab_id_dict):
        ...
        while d >= d_d_min:
            ...
            if os.path.exists(msg_fp):
                ...
                try:
                    with open(msg_fp, 'r', encoding='utf-8') as tweet_f:
                        for line in tweet_f:
                            try:
                    ...
                    logger.warning(f"Error reading tweet file {msg_fp}: {e}")
    ...
    def sample_gen_from_one_stock(self, vocab_id_dict, stock_id_dict, s, phase):
        ...
        try:
            with open(stock_movement_path, 'r', encoding='utf-8') as movement_f:
    ...
    def batch_gen(self, phase):
        ...
        # Filter out None generators
        generators = [gen for gen in generators if gen is not None]
        # logger.info('{0} Generators prepared...'.format(len(generators)))

        while generators:  # Continue while we have active generators
            ...
            if sample_id == 0:  # No more samples available
                break
    ...
    def sample_mv_percents(self, phase):
        ...
        for s in stock_symbols:
            ...
            try:
                with open(stock_mv_path, 'r', encoding='utf-8') as movement_f:
                    for line in movement_f:
                        try:
                            data = line.strip().split('\t')
                            if len(data) < 2:
                ...
                logger.warning(f"Stock movement file not found: {stock_mv_path}")
                continue
            except Exception as e:
                logger.error(f"Error reading stock movement file {stock_mv_path}: {e}")
                continue
    ...
    def init_word_table(self):
        ...
        # Python 3 fix: use != instead of 'is not' for string comparison
                ...
                with io.open(self.glove_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        try:
                            ...
                            if len(tuples) < self.word_embed_size + 1:
                                continue  # Skip malformed lines

                            word = tuples[0]
                            embed = [float(embed_col) for embed_col in tuples[1:self.word_embed_size+1]]
                            ...
                            # Skip malformed embedding lines
                            continue

                logger.info('ASSEMBLE: word table #replacement: {}'.format(n_replacement))
            except FileNotFoundError:
                logger.warning(f"GloVe file not found: {self.glove_path}, using random embeddings")
            except Exception as e:
                logger.error(f"Error reading GloVe file {self.glove_path}: {e}, using random embeddings")
